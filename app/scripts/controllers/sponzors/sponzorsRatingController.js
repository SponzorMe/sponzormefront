...
(function() {
  ...
  function SponzorsRatingController($scope, $translate, userRequest, ngDialog, $location, $rootScope, $localStorage, $routeParams, sponzorshipRequest) {
    if($rootScope.userValidation('1'), $routeParams.sponzorshipId){
      ...
      sponzorshipRequest.oneSponzorship($routeParams.sponzorshipId).success(function(sData){
        ...
        $scope.rating = {'sponzorship_id':sData.data.SponzorEvent.id, 'type':1, 'sponzor_id':sData.data.Sponzor.id, 'organizer_id':sData.data.Organizer.id};
